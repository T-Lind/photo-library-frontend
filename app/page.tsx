                                    ...
                                    <SelectItem key={person.people_id} value={person.people_id.toString()}>
                                        <div className="flex items-center">
                                            <Avatar className="h-12 w-12 mr-3">
                                                <AvatarImage src={api.getPersonFaceUrl(person.people_id)} alt={person.name || `Person ${person.people_id}`} />
                                                <AvatarFallback>{(person.name && person.name.charAt(0)) || person.people_id}</AvatarFallback>
                                            </Avatar>
                                            <span className="flex-grow">{person.name || `Person ${person.people_id}`}</span>
                                            <Check
                                                className={cn(
                                                    "ml-auto h-4 w-4",
                    ...
                    {selectedPeople.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                            {selectedPeople.map(person => (
                                <div key={person.people_id} className="flex items-center bg-orange-200 rounded-full px-3 py-1">
                                                ...
                                                {image.people_ids.map((peopleId) => {
                                                    ...
                                                    return person ? (
                                                        <Avatar key={peopleId} className="h-10 w-10">
                                                            <AvatarImage src={api.getPersonFaceUrl(peopleId)} alt={person.name || `Person ${peopleId}`} />
